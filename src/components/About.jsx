...
const mobileFromLeft = {
  hidden: { opacity: 0, x: -15, transition: { duration: 0.5 } },
  visible: {
    opacity: 1,
    x: 0,
            ...
            <div className="md:hidden pt-1">
              <div className="flex flex-wrap gap-1">
                {[...row1Tags, ...row2Tags].map((tag, i) => (
                  <span key={`mob-${i}`} className={`inline-flex items-center gap-0.5 px-1.5 py-0.5 text-[8px] font-medium border rounded-full whitespace-nowrap ${tag.color}`}>
                    {tag.icon}
                    {tag.label}
                  </span>
                ))}
