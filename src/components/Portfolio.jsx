    ...
    >
      ...
      >
        <div className={`h-56 md:h-72 bg-gradient-to-br ${project.gradient} relative`}>
          ...
          <button onClick={onClose} className="absolute top-4 right-4 w-10 h-10 rounded-full bg-black/40 backdrop-blur-sm flex items-center justify-center hover:bg-black/60 transition-colors">
            <svg className="w-5 h-5 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
            </svg>
          </button>
          <div className="absolute bottom-4 left-6">
            <span className="font-display text-4xl font-bold text-white">{project.title}</span>
          </div>
        </div>
        <div className="p-6 md:p-8 space-y-5">
          <div className="flex flex-wrap gap-2">
            {project.tags.map((tag) => (
              <span key={tag} className="px-3 py-1 text-xs font-medium bg-dark-300 rounded-full text-white/50">{tag}</span>
            ))}
          </div>
          <p className="text-white/60 text-base leading-relaxed">{project.fullDesc}</p>
  ...
  return (
    <section id="portfolio" className="section-panel">
      ...
      >
        ...
        <motion.div variants={fadeUp} className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-5">
            ...
            >
              <div className={`h-40 md:h-48 bg-gradient-to-br ${project.gradient} relative overflow-hidden`}>
                ...
                <div className="absolute top-4 left-4">
                  <span className="text-white/80 font-display text-3xl font-bold">0{i + 1}</span>
                </div>
              ...
              <div className="p-5 md:p-6">
                <h3 className="font-display text-lg font-semibold text-white group-hover:text-accent transition-colors mb-2">{project.title}</h3>
                <p className="text-white/40 text-sm leading-relaxed mb-4 line-clamp-2">{project.desc}</p>
                <div className="flex flex-wrap gap-2">
                  {project.tags.slice(0, 3).map((tag) => (
                    <span key={tag} className="px-2.5 py-1 text-[10px] md:text-xs font-medium bg-dark-300 rounded-md text-white/40">{tag}</span>
                  ))}
                  {project.tags.length > 3 && (
                    <span className="px-2.5 py-1 text-[10px] md:text-xs font-medium bg-dark-300 rounded-md text-white/30">+{project.tags.length - 3}</span>
                  )}
                </div>
              </div>
            </motion.div>
